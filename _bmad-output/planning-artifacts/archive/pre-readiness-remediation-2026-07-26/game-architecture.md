...
epics: '_bmad-output/planning-artifacts/gdds/gdd-outsurvive-2026-07-22/epics.md'
brief: null
...
6. **Reconnect/AFK/mobile lifecycle:** avatar protection, timeout và outcome chưa được GDD khóa; kiến trúc phải định nghĩa owner/API cho interruption, background, process eviction và network handoff trước E4-S8/S9/E6-S6.
...
| ARC-A09 | Invite, voice và reconnect có thể ẩn sau provider adapter | Trung bình | Cao | Khóa interface, token ownership và failure behavior trước E6-S4/S6 |
...
- terrain tile index;
- coastline và `WaterBody`;
- `PoiDefinition` và compound;
- `RoadGraph`;
- `BuildingPlacement`;
- cover, loot region, vehicle spawn và dock;
- navigation/connectivity metadata;
- validation rules.
...
- Bus tối thiểu: Master, UI, Ambience, Footsteps, Weapons, Vehicles và Voice.
...
| Check | Result | Notes |
|---|---|---|
| Decision compatibility | PASS | 20 ADR nhất quán sau khi sửa database production baseline và cặp Agones/Kubernetes |
...
| Epic mapping | PASS | 9/9 Epic có module, boundary và pattern triển khai tương ứng |
...
| E5 — Đảo Vọng và phương tiện | Multi-grid world cells, map schema, vehicles/water, content bake; N-03, N-06 |
| E6 — Multiplayer cạnh tranh | Protocol, authority, allocation, relevance, signed input family; N-01, N-04, N-05, S-06 |
| E7 — Độ rõ và tiếp cận | Presenter/view state, UI/audio/voice/platform adapters; S-07 |
| E8 — Hồ sơ và tính toàn vẹn | Nakama/CockroachDB control plane, account/profile policy và config rules; S-09 |
| E9 — Ổn định và chứng nhận | CI, infrastructure, test harness, observability, security và five-platform gates; S-10 |

**Decision coverage — 20/20:** runtime, network, world, terrain, map, simulation, lifecycle, gameplay data, streaming, persistence, control plane, fleet, platform/input, UI, audio/voice, AI, security/replay, build/versioning, observability và dependency promotion đều có ADR.
...
**Overall result:** PASS. Kiến trúc hoàn chỉnh ở mức thiết kế; các giả định về scale, mobile, network, world streaming, water/boat, voice và five-platform production vẫn phải vượt các empirical validation gate đã chỉ định trước khi khóa production content hoặc phát hành v1.
...
2. Chạy Implementation Readiness trên GDD, 9 Epic hiện có, UX và Architecture; không tạo lại Epic trừ khi kiểm định phát hiện thiếu hoặc mâu thuẫn.
3. Scaffold domain-driven monorepo cùng dự án Godot Standard sạch theo cây thư mục đã khóa; chưa promote asset từ `prepare-asset/`.
