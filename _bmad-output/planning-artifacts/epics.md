...
- **BẮT BUỘC CHO STORY 1.1:** Chỉ khởi tạo dự án Godot Standard sạch + local bootstrap; module boundary, CI/toolchain và five-platform expansion thuộc Story 1.2–1.5. Không kế thừa starter kit hoặc framework gameplay bên thứ ba.
...
- World schema phát triển theo consumer: Story 5.1 chỉ có terrain/water dependency tối thiểu; Story 5.3/5.4/5.5/5.7 mới bổ sung cell, POI/road, building và weather fields kèm migration/validator. Full `MapDefinition` chỉ hoàn tất khi các consumer tương ứng đã tồn tại.
...
- Animation nhân vật chỉ là presentation: Story 1.9 sở hữu intake/rig, Story 1.10 locomotion graph, Story 1.11 aim/lean layer và Story 1.12 parity/package evidence. Weapon/reload presentation chỉ được tạo khi Epic 2 có consumer; không reserve feature tương lai trong cùng Story.
...
- Backlog tách clean bootstrap (1.1), module boundary (1.2), dependency/SBOM (1.3), canonical CI smoke (1.4), five-platform build smoke/signing boundary (1.5) và quarantine (1.6); release Epic chỉ qualification/hardening.
...
- Reconnect policy đã khóa tại Story 6.15: party clear Ready; pre-aircraft grace 60 giây; live grace 120 giây; neutral input + vulnerable avatar; timeout sau aircraft direct-eliminate/drop/defeat; connected-AFK chỉ telemetry, không protection/autopilot/kick.
- Input policy đã khóa tại Story 1.20: gameplay binding dùng physical key identity, text entry dùng logical/Unicode, keycap dùng localized OS label; aim dùng captured unaccumulated relative delta và explicit fallback capability.
- Map policy đã khóa tại Story 8.8: Full Map north-up, minimap player-heading-up, zoom 0,75×–6× và pan clamp giữ ≥20% authored bounds; damage cue vẫn world-camera-relative.
- Spectator policy đã khóa tại Story 4.9/6.12/6.13: teammate-follow, được team voice theo mute/block/PTT, không được phát world/context/Map ping hoặc marker mới sau elimination.
...
### GDD Readiness Trace Addendum

Các ID `GDD-IR-FR*` dưới đây là ID truy vết được gán trong Implementation Readiness 2026-07-26 cho yêu cầu GDD 1.1.0 chưa có mã gốc. Chúng không thay thế FR1–FR110 nội bộ; chúng đóng 12 finding semantic còn thiếu/một phần:

| GDD readiness ID | Requirement contract | Story owner | Required evidence |
|---|---|---|---|
| GDD-IR-FR55 | Combat range 0–25 / 40–150 / 150–400 / 400–800 m theo space type | 5.6, 7.7 | Sightline histogram + collision witness |
| GDD-IR-FR57 | Cover tĩnh; không destructibility/digging/building | 5.5, 5.6, 7.4 | Scope validator + client/server LOS parity |
| GDD-IR-FR59 | Không full-night; bốn outfit detection delta ≤5% | 5.7, 7.9 | Controlled detection study desktop/mobile |
| GDD-IR-FR65 | Standard là luật chính; ranked/custom/tournament/FPP-only deferred | 9.7 | Product capability/route/config scan |
| GDD-IR-FR70 | Progression curve 0–3 / 3–10 / 10–22 / 22+ phút | 4.13 | Segment telemetry + balance finding |
| GDD-IR-FR75 | Mỗi weapon archetype có strength và disadvantage | 2.3 | Manifest validator + balance metric |
| GDD-IR-FR76 | POI 800–1.500 m, ≥2 ground route, non-vehicle escape ≤70% chậm hơn | 5.4, 7.3, 7.7 | Route graph/travel-time witness |
| GDD-IR-FR77 | Roof counterplay, ≥80% window family, penetration visual language | 5.5, 7.4, 7.5 | Building-kit/placement validator |
| GDD-IR-FR78 | Environmental navigation không phụ thuộc minimap | 7.8 | No-minimap traversal study |
| GDD-IR-FR79 | Restrained realism, ordinary survivor, fictional weapon identity | 1.9, 2.3, 7.4 | Content/art contract review |
| GDD-IR-FR85 | Footstep/gunshot range, surface spectrum và masking | 7.8, 8.12 | Propagation/range/masking matrix |
| GDD-IR-FR93 | Post-release P1–P4 + metric; reject/tách Standard nếu giảm pillars | 9.7 | Proposal schema + adversarial audit |

**Coverage gate:** 93/93 GDD requirements phải map tới Story + executable evidence. Không được coi FR Coverage Map nội bộ 110/110 là thay thế cho semantic trace này.
...
**Sprint-entry dependencies:** Story 1.23 cho Story 2.1; các Story sau dùng dependency ID riêng.
...
**Sprint-entry dependencies:** Story 2.3 và Story 2.15 cho kernel Story 3.1; không chờ hoàn tất toàn Epic.
...
**Sprint-entry dependencies:** Story 1.23, Story 2.15 và Story 3.12 cho Story 4.1.

### Epic 5: Khám phá và thoát hiểm trong vùng thử nghiệm

Người chơi có thể tự chọn đi bộ, bơi hoặc dùng phương tiện để tiếp cận rồi thoát khỏi hai POI qua nhiều route hợp lệ trong vùng thử nghiệm 2×2 km, không gặp lỗi streaming, cover hoặc gameplay parity.

**FRs covered:** FR51–FR55, FR84–FR89  
**Sprint-entry dependencies:** Story 1.2–1.5 và Story 4.13 cho Story 5.1.
...
**Sprint-entry dependencies:** Story 1.2–1.5 và Story 2.1 cho Story 6.1; mỗi online capability có dependency Story riêng.
...
**Sprint-entry dependencies:** Story 5.13 và Story 6.20 phải đạt trước Story 7.1.
...
**Sprint-entry dependencies:** Story 1.7 cho Story 8.1; các surface sau ghép với Story capability sở hữu theo Delivery Waves.
...
**Sprint-entry dependencies:** Story 4.11, Story 6.17 và Story 8.19 cho Story 9.1; các Story khác dùng dependency ID riêng.
...
**Sprint-entry dependencies:** Story 1.3–1.5, Story 8.19 và Story 9.8 cho Story 10.1; qualification platform sau dùng dependency ID riêng.

## Delivery Waves và Sprint-entry Policy

Epic number là nhóm product capability, **không phải yêu cầu triển khai tuần tự toàn Epic**. Sprint Planning phải dùng dependency Story/contract bên dưới:

1. **Foundation Wave:** Story 1.1–1.8. Scaffold, module boundary, dependency/SBOM, CI artifact family, quarantine, presenter/Theme và StringId/localization phải hoàn tất trước feature consumer tương ứng.
2. **Playable Survivor Wave:** Story 1.9–1.23; Story 1.7–1.8 có thể chạy song song với character/input foundation.
3. **Capability-paired UX:** HUD đi cùng combat/match state (Story 2.12, 4.8, 8.5–8.6); Inventory đi cùng resource flow (Story 3.2–3.6, 8.7); Map đi cùng flight/zone/ping (Story 4.4, 4.8, 6.13, 8.8); Lobby/Party đi cùng online control plane (Story 6.1–6.7, 8.4); audio/voice hardening đi cùng Story 6.12–6.13 và 8.12–8.14.
4. **Online Operations Wave:** Story 6.8 allocator → 6.9 container → 6.10 observability → 6.11 secure admission; không chờ release Epic.
5. **Production/Release Wave:** full-map content chỉ mở sau Story 5.13 và 6.20; platform qualification dùng CI/SBOM/container/observability đã tồn tại, không tạo lại control kỹ thuật ở cuối.

Một Story chỉ được chuyển `ready-for-dev` khi:

- dependency là Story ID + artifact/contract đã tồn tại, không dùng “xong cả Epic” làm điều kiện mơ hồ;
- không còn owner decision/ADR open cho behavior Story tiêu thụ;
- có đúng một capability hoặc một evidence/promotion outcome, estimate tối đa 5 ngày làm việc;
- nếu estimate vượt 5 ngày, có nhiều capability hoặc gate chứa feature work, Story phải split trước Sprint Planning;
- Verification/Promotion Story chỉ tổng hợp evidence đã sinh; failure quay về Story sở hữu feature.
...
**Delivery Type:** Enabler — mở Story 1.2–1.8 và liên kết trực tiếp tới playable movement slice.
...
**Module/File Ownership:** `game/project.godot`, bootstrap kernel, smoke scene và test baseline.
**Platform Applicability:** Local editor và Linux headless import.

**Acceptance Criteria:**

**Given** repository chưa có project gameplay  
**When** scaffold được tạo và chạy bằng Godot Standard `4.7.1-stable`  
**Then** typed GDScript project khởi động/đóng sạch, hiển thị `build_id`, engine version và lifecycle state  
**And** không cần backend, network live, starter kit hoặc framework gameplay bên thứ ba.

**Error/Recovery Path:** Engine/template mismatch fail-closed trước import và không ghi artifact hợp lệ giả.
**Test Boundary:** Clean-checkout headless import, bootstrap unit smoke và local launch/quit.
**Definition of Done:** Clean checkout tạo và chạy được smoke scene bằng đúng engine/checksum.
...
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** NFR31, NFR55; Architecture domain-driven monorepo.
**Depends on:** Story 1.1.
**Blocks:** Story 1.6–1.7, Story 1.13 và mọi shared simulation Story.
**Module/File Ownership:** `game`, `native`, `backend`, `infrastructure`, `content-source`, `prepare-asset`, `test-harness`, `docs` và boundary scanner.
**Platform Applicability:** Repository/CI.

**Acceptance Criteria:**

**Given** module tree đã tạo  
**When** dependency scan chạy  
**Then** `shared` không phụ thuộc client, server, UI, renderer, platform SDK hoặc backend implementation  
**And** platform/provider code chỉ xuất hiện sau typed port/adapter.

**Error/Recovery Path:** Import/ownership sai trả file + forbidden edge và chặn merge.
**Test Boundary:** Static dependency graph, forbidden-import fixtures và composition-root smoke.
**Definition of Done:** Boundary graph có owner, rule và automated fail-closed evidence.
...
**Delivery Type:** Enabler — consumer là Story 1.4/1.5 và mọi release qualification.
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** NFR26, NFR31, NFR49, NFR55; ADR-18/20.
**Depends on:** Story 1.1.
**Blocks:** Story 1.4–1.6 và Story 10.1–10.11.
**Module/File Ownership:** Version/checksum manifest, dependency policy, SBOM generator và upgrade ADR template.
**Platform Applicability:** Tooling, năm client, Linux headless.

**Acceptance Criteria:**

**Given** engine, export template, plugin, SDK hoặc tool  
**When** dependency resolution/build chạy  
**Then** version + checksum phải khớp lock, SBOM có license/source/hash và mismatch fail-closed  
**And** upgrade chỉ được chấp nhận bằng ADR cùng impacted-platform smoke matrix.

**Error/Recovery Path:** Dependency không khóa hoặc CVE/license chưa quyết định không được promote.
**Test Boundary:** Lock drift, checksum corruption, SBOM completeness và upgrade-ADR schema.
**Definition of Done:** Baseline lock/SBOM được tạo tái lập và dùng bởi build smoke.

### Story 1.4: Reproducible canonical build smoke

As a developer,
I want một artifact client chuẩn được build lặp lại trong CI,
So that gameplay changes có feedback build sớm.

**Delivery Type:** Enabler — consumer là mọi Story gameplay.
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** NFR31, NFR49, NFR55; Architecture build manifest.
**Depends on:** Story 1.1–1.3.
**Blocks:** Story 1.5–1.6 và Story 1.13.
**Module/File Ownership:** CI base workflow, canonical Linux client preset, artifact manifest/checksum và launch smoke.
**Platform Applicability:** Linux x64 client trên runner chuẩn.

**Acceptance Criteria:**

**Given** clean workspace và locked toolchain  
**When** CI canonical build chạy hai lần  
**Then** artifact family có cùng build inputs/manifest, headless import/unit/boundary/launch smoke pass  
**And** artifact dở dang, secret, generated cache hoặc direct `print()` bị chặn.

**Error/Recovery Path:** Build/test fail không publish; log typed và không chứa secret.
**Test Boundary:** Clean build, repeat build manifest comparison, launch/quit và failure publication guard.
**Definition of Done:** Canonical artifact tái tạo được và trở thành required check.

### Story 1.5: Five-platform build-smoke expansion và signing boundaries

As a platform engineer,
I want cùng source tạo development artifact trên mọi target,
So that incompatibility nền tảng được phát hiện trước gameplay expansion.

**Delivery Type:** Enabler — mở five-platform parity gate và release qualification.
**Sizing:** M — tối đa 5 ngày; signing production không nằm trong Story này.
**Source Requirements:** FR13; NFR31, NFR49, NFR55.
**Depends on:** Story 1.4.
**Blocks:** Story 1.12, Story 1.23 và Story 10.1–10.5.
**Module/File Ownership:** Export presets, platform CI jobs, isolated macOS runner contract và signing-secret boundary.
**Platform Applicability:** Windows x64, Linux x64, macOS Universal 2, Android ARM64, iOS ARM64, Linux headless.

**Acceptance Criteria:**

**Given** canonical build workflow  
**When** matrix mở rộng chạy  
**Then** tạo sáu development artifacts với manifest/checksum và launch-smoke evidence cho từng platform family  
**And** signing key/provisioning/store credential chỉ là external secret reference, không nằm trong repo/log/prompt/MCP.

**Error/Recovery Path:** SDK/runner/signing-capability thiếu trả explicit unavailable và fail target; không dùng waiver để pass matrix.
**Test Boundary:** Export-preset validation, artifact inspection và launch smoke trên runner/device phù hợp.
**Definition of Done:** Sáu artifact dev có evidence; release signing vẫn được chứng minh trong Story 10.1–10.5.

### Story 1.6: Asset quarantine và promotion validation

As a content engineer,
I want asset chưa duyệt bị cô lập khỏi runtime,
So that prototype không vô tình ship nội dung thiếu license hoặc contract kỹ thuật.

**Delivery Type:** Enabler — consumer đầu tiên là Story 1.9 và Story 1.22.
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** NFR31, NFR35, NFR55; content-promotion contract.
**Depends on:** Story 1.2–1.5.
**Blocks:** Story 1.9–1.12, Story 1.22 và mọi production content Story.
**Module/File Ownership:** `prepare-asset`, promotion manifest/schema, provenance/license validator và import guard.
**Platform Applicability:** Authoring, CI và package inspection.

**Acceptance Criteria:**

**Given** asset ở `prepare-asset/`  
**When** headless import/build chạy  
**Then** asset không tự vào Godot/content manifest; chỉ stable ID có URL/author/license/date/archive + per-file checksum mới được promote  
**And** failed promotion giữ signed manifest trước, không sửa một phần.

**Error/Recovery Path:** Asset sai quay lại quarantine với file/field reason; placeholder hợp lệ tiếp tục chạy.
**Test Boundary:** Direct-import guard, provenance/checksum fixtures và package scan.
**Definition of Done:** Quarantine fail-closed và một fixture hợp lệ/không hợp lệ chứng minh promotion path.

### Story 1.7: Presenter, Theme và accessible UI primitive foundation

As a UI developer,
I want presenter/view-state và Theme base xuất hiện ngay sau scaffold,
So that gameplay UI không hard-code packet, style hoặc accessibility.

**Delivery Type:** Enabler — consumer là HUD/Inventory/Map/Party surface theo từng capability.
**Sizing:** M — tối đa 5 ngày.
**Source Requirements:** UX-DR1–UX-DR18, UX-DR35–UX-DR40; ADR-14.
**Depends on:** Story 1.1–1.2.
**Blocks:** Story 8.1–8.2 và mọi player-facing presenter.
**Module/File Ownership:** `game/src/client/ui/{presenters,view_states,components}`, base `Theme`, focus/error/loading primitives.
**Platform Applicability:** Năm client.

**Acceptance Criteria:**

**Given** mock domain state  
**When** base presenter render immutable `ViewState`  
**Then** primitive nhận state/local action qua typed boundary, không đọc packet/entity/backend trực tiếp  
**And** focus, disabled, loading, empty, error và safe-area states có keyboard/Touch smoke.

**Error/Recovery Path:** Missing state/token hiển thị explicit fallback và fail snapshot, không silently default tactical information.
**Test Boundary:** Presenter unit, boundary scan, Theme token snapshot và keyboard/Touch focus smoke.
**Definition of Done:** Một vertical primitive render được Việt/Anh placeholder và desktop/mobile layout without gameplay dependency.

### Story 1.8: StringId và Việt/Anh localization scaffold

As a UI developer,
I want mọi chuỗi đi qua locale catalog có fallback xác định,
So that surface gameplay không tích lũy hard-coded text.

**Delivery Type:** Enabler — consumer là mọi player-facing Story.
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** UX-DR19, UX-DR37; Architecture localization boundary.
**Depends on:** Story 1.7.
**Blocks:** Story 8.3–8.19 và mọi production UI string.
**Module/File Ownership:** `StringId`, `LocaleCatalog`, Việt/Anh catalogs, formatting policy, Noto license/font smoke.
**Platform Applicability:** Năm client.

**Acceptance Criteria:**

**Given** presenter phát `StringId` + typed arguments  
**When** locale `vi` hoặc `en` resolve  
**Then** text dùng catalog immutable, English là fallback có telemetry missing-key và gameplay/domain không phát display string  
**And** number/date/plural, OS key label alias và Noto Vietnamese glyph có golden fixtures.

**Error/Recovery Path:** Missing/invalid entry hiển thị safe debug ID ngoài release và chặn localization gate; không trả chuỗi rỗng.
**Test Boundary:** Catalog schema, missing/fallback, formatting, glyph snapshot và oversized-label fixtures.
**Definition of Done:** Việt/Anh round-trip đạt và hard-coded player-facing string scan hoạt động.

### Story 1.9: Prototype Survivor asset intake và humanoid rig

As a character artist,
I want một placeholder survivor có nguồn gốc và rig ổn định,
So that movement presentation dùng asset hợp lệ thay vì giả định production art.

**Delivery Type:** Enabler — consumer là Story 1.10–1.12 và player movement slice.
**Sizing:** M — tối đa 5 ngày.
**Source Requirements:** FR13; presentation slice FR14; GDD-IR-FR79 restrained-realism/ordinary-survivor contract; NFR31, NFR35.
**Depends on:** Story 1.6.
**Blocks:** Story 1.10–1.12.
**Module/File Ownership:** `content-source/characters`, license records, approved character asset, skeleton/bone-map validator.
**Platform Applicability:** Năm client; headless chỉ giữ stable humanoid contract.

**Acceptance Criteria:**

**Given** Quaternius candidate source  
**When** model `Regular` và required clips được promote  
**Then** URL, CC0 copy, retrieval/checksums/stable ID đầy đủ; scale/orientation/rest pose/bone map/sockets đạt humanoid contract  
**And** silhouette là survivor bình thường, không hero/fantasy/production identity hoặc camouflage advantage.

**Error/Recovery Path:** Provenance/rig sai giữ asset trong quarantine và placeholder trước vẫn dùng được.
**Test Boundary:** Provenance, skeleton/bone-map/socket và silhouette review.
**Definition of Done:** Approved humanoid rig import được và không kéo presentation asset vào headless package.

### Story 1.10: Locomotion animation state graph

As a người chơi,
I want tư thế vận động chuyển rõ và ổn định,
So that tôi đọc được hành động của mình và đối thủ.

**Delivery Type:** Player-facing capability.
**Sizing:** M — tối đa 5 ngày.
**Source Requirements:** presentation FR14, FR17–FR18, FR20; NFR35.
**Depends on:** Story 1.9.
**Blocks:** Story 1.11–1.12, Story 1.14–1.18.
**Module/File Ownership:** Character `AnimationTree`, locomotion clips/state graph và traversal tests.
**Platform Applicability:** Năm client.

**Acceptance Criteria:**

**Given** idle/walk/jog/sprint/crouch/prone/air/vault/swim/dive visual states  
**When** graph chuyển trạng thái  
**Then** transition không pop/kẹt; clip thiếu được author cùng skeleton + derived-edit record  
**And** root displacement ≤0,01 m, animation không điều khiển authoritative transform/capsule/trajectory.

**Error/Recovery Path:** Missing/invalid clip dùng explicit safe pose và fail promotion, không tạo state mất hình.
**Test Boundary:** State traversal, transition snapshot và root-motion tolerance.
**Definition of Done:** Locomotion graph chạy đủ state hiện tại, không reserve weapon/reload implementation chưa có consumer.

### Story 1.11: Upper-body aim và lean presentation layer

As a người chơi,
I want aim/lean pose không phá chuyển động chân,
So that hướng hành động vẫn đọc được khi di chuyển.

**Delivery Type:** Player-facing capability.
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** presentation FR18, FR33; NFR35.
**Depends on:** Story 1.10.
**Blocks:** Story 1.12, Story 1.18 và combat presentation Story 2.10.
**Module/File Ownership:** Bone-filter/procedural aim + lean layer và pose fixtures.
**Platform Applicability:** Năm client.

**Acceptance Criteria:**

**Given** `aim_pitch` và `lean_amount` hợp lệ  
**When** layer blend trên locomotion  
**Then** upper body phản ánh pitch/lean, chân và root không trượt  
**And** chỉ expose typed inputs hiện dùng; weapon action/reload layer được tạo trong Story consumer Epic 2.

**Error/Recovery Path:** Invalid pose input clamp + diagnostic; layer failure quay về locomotion pose.
**Test Boundary:** Pose extremes, locomotion blend và root/capsule independence.
**Definition of Done:** Aim/lean visual state đạt parity fixture trước camera/weapon integration.

### Story 1.12: Character parity và headless package evidence

As a build engineer,
I want character contract giống nhau trên desktop/mobile và tối giản trên server,
So that presentation variant không đổi gameplay silhouette hoặc package authority.

**Delivery Type:** Verification — chỉ tạo parity/package evidence, không triển khai animation feature mới.
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** FR13; NFR31, NFR35, NFR49, NFR55.
**Depends on:** Story 1.5, Story 1.9–1.11.
**Blocks:** Story 1.22–1.23 và combat/world content promotion.
**Module/File Ownership:** Character parity matrix, package-content tests và signed evidence.
**Platform Applicability:** Năm client và Linux headless.

**Acceptance Criteria:**

**Given** cùng visual-state/tick fixture  
**When** desktop/mobile variants và headless package được kiểm tra  
**Then** skeleton timing, silhouette, sockets và gameplay contract tương đương; chỉ LOD/material/shader quality khác  
**And** server không chứa texture/mesh/animation/presentation archive.

**Error/Recovery Path:** Platform/package mismatch block promotion và giữ artifact family trước.
**Test Boundary:** Five-platform visual smoke, silhouette/socket parity và server package scan.
**Definition of Done:** Signed parity report tồn tại; không có feature implementation ẩn trong gate.

### Story 1.13: GameplayCommand ngữ nghĩa và fixed-tick input pipeline
...
**Blocks:** Story 1.14–1.21 và mọi prediction/authority story.
...
### Story 1.14: Ground locomotion và sprint trade-off
...
**Depends on:** Story 1.10, Story 1.13.

**Blocks:** Story 1.15–1.17, Story 1.22, Story 2.11.
...
### Story 1.15: Crouch, prone và lean có collision guard
...
**Depends on:** Story 1.14.

**Blocks:** Story 1.18, Story 1.22, Story 2.6, Story 2.11.
...
### Story 1.16: Jump, vault và fall damage authoritative
...
**Depends on:** Story 1.14–1.15.

**Blocks:** Story 1.22 và combat movement validation.
...
### Story 1.17: Swimming, diving và WaterVolume
...
**Depends on:** Story 1.13–1.16.

**Blocks:** Story 1.22, Story 5.13.
...
### Story 1.18: Camera TPP, shoulder swap, ADS và anti-peek
...
**Depends on:** Story 1.11, Story 1.14–1.17.

**Blocks:** Story 2.3–2.10 và Story 4.9.
...
### Story 1.19: Context interaction và arbitration
...
### Story 1.20: Keyboard/Mouse bindings, remap và sensitivity contract
...
**Depends on:** Story 1.13, Story 1.18–1.19.

**Blocks:** Story 8.9, Story 8.19.
...
**Then** gameplay action phải lưu physical key identity, text entry dùng logical/Unicode input và keycap lấy localized OS label/alias từ Story 1.8  
**And** label/display text không bao giờ là gameplay identity.

**Given** pointer bị capture cho camera/aim  
**When** mouse motion được lấy mẫu  
**Then** adapter dùng unaccumulated captured relative delta (`Input.use_accumulated_input = false`) và delta không bị stretch/UI scale biến đổi  
**And** nếu platform không cung cấp capability tương đương, Settings phải công bố fallback relative-input state thay vì silently đổi sensitivity/acceleration.
...
### Story 1.21: Touch landscape, multitouch và interruption safety
...
**Depends on:** Story 1.13, Story 1.18–1.20.

**Blocks:** Story 8.11, Story 8.19, Story 10.4–10.5.
...
### Story 1.22: Movement Validation Course và promotion asset bề mặt/âm thanh
...
**Delivery Type:** Enabler/validation course — tạo reusable fixture và promoted test assets, không che movement feature.
**Sizing:** M — tối đa 5 ngày.

**Depends on:** Story 1.6, Story 1.9–1.21.

**Blocks:** Story 1.23 và các movement/combat regression gate.
...
### Story 1.23: Epic 1 five-platform parity gate
...
**Delivery Type:** Verification/promotion — chỉ tổng hợp automated evidence và quyết định mở gate; không chứa feature implementation.
**Sizing:** M — tối đa 5 ngày cho evidence matrix; feature failure quay về Story sở hữu.

**Source Requirements:** FR13–FR24, FR56–FR58; NFR15, NFR31–NFR35, NFR49.

**Depends on:** Story 1.1–1.22.

**Blocks:** Story 2.1, Story 5.2 và các production-content Story dùng character/surface manifests.
...
**Depends on:** Story 1.23.

**Blocks:** Story 2.2, Story 2.11–2.15, Story 6.11.
...
**Source Requirements:** FR59–FR62; GDD-IR-FR75, GDD-IR-FR79; NFR23–NFR24, NFR31, NFR55.

**Depends on:** Story 2.2.

**Blocks:** Story 2.4–2.10, Story 3.1.
...
**Given** mỗi weapon archetype và production-facing weapon identity  
**When** balance/art validator chạy  
**Then** archetype phải khai báo ít nhất một tình huống mạnh và một tình huống bất lợi có metric/range kiểm chứng  
**And** tên/hình dáng/âm thanh giữ fictional identity và restrained realism, không sao chép trade dress/vũ khí tham chiếu hoặc tạo silhouette pay-to-confuse.
...
**Blocks:** Story 2.9–2.13, Story 4.6.
...
**Blocks:** Story 2.12–2.15, Story 6.18.
...
**Blocks:** Story 3.1, Story 4.1, Story 6.18.
...
### Story 3.1: Stable ItemId và immutable catalog kernel

As a developer,
I want một catalog kernel chỉ sở hữu identity và type discriminator,
So that từng capability bổ sung definition vừa đủ khi thực sự tiêu thụ.

**Delivery Type:** Enabler — consumer đầu tiên là Story 3.2; không định nghĩa trước toàn item domain.
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** NFR27, NFR31, NFR55; ADR-08/S-04.

**Depends on:** Story 2.3, Story 2.15.

**Blocks:** Story 3.2, Story 3.4 và Story 3.7.

**Module/File Ownership:** Shared `ItemId`, `ItemKind`, catalog header/version/hash, uniqueness validator và read-only lookup port.

**Platform Applicability:** Năm client và Linux headless server dùng cùng gameplay hash.

**Acceptance Criteria:**

**Given** minimal catalog authoring source  
**When** bake  
**Then** mỗi entry chỉ bắt buộc stable `ItemId`, `ItemKind`, schema version và definition reference do Story consumer sở hữu  
**And** không thêm trước slot/capacity/heal/throwable fields chưa có consumer; display name/file path không làm protocol identity.
...
**Definition of Done:** Catalog kernel immutable/versioned/cùng hash client-server; equipment, healing và throwable schema chưa tồn tại trước Story consumer.
...
**Given** Story 3.2 là consumer đầu tiên của equipment/capacity data  
**When** equipment definitions được bổ sung vào catalog  
**Then** chỉ weapon/ammo/attachment/equipment/backpack fields cần cho slot/capacity invariant được bake  
**And** schema extension có version, range/reference validation và client/server hash parity.
...
**Blocks:** Story 3.4–3.6, Story 3.9–3.11, Story 8.7.
...
**Given** healing/boost capability bắt đầu tiêu thụ item data  
**When** consumable definitions được thêm  
**Then** duration/effect/HP cap/boost/interrupt policy thuộc Story 3.6, không thuộc catalog kernel Story 3.1  
**And** invalid range/reference chặn manifest trước runtime.
...
**Given** throwable inventory là consumer đầu tiên  
**When** throwable definitions được bổ sung  
**Then** capacity cost, selected-slot compatibility và preview parameters tối thiểu thuộc Story 3.10  
**And** fuse/radius/effect chỉ được bổ sung bởi Story 3.11.
...
### Story 3.11: Four authoritative throwable archetypes
...
**Delivery Type:** Player-facing capability.
**Sizing:** M — tối đa 5 ngày; nếu một archetype không cùng simulation primitive thì split trước `ready-for-dev`.

**Depends on:** Story 3.10 và Story 2.5–2.12.

**Blocks:** Story 3.12 và Story 4.1.

**Module/File Ownership:** Throwable definitions/simulation/effects, server area queries, presentation pooling và archetype fixtures.
...
**And** fuse/radius/duration/effect fields được Story 3.11 mở rộng từ kernel, damage/status/area ownership do server xác nhận.
...
**Test Boundary:** Mỗi archetype golden, fuse/radius/duration, stacked effects, pooling reset và impairment transactions.

**Definition of Done:** Bốn throwable hoạt động authoritative và có feature evidence riêng; không chứa Epic promotion decision.

### Story 3.12: Survival-resource promotion evidence

As a QA producer,
I want evidence tái lập cho toàn bộ loot/inventory/resource flow,
So that Epic 4 chỉ bắt đầu khi scarcity và mutation authority đã được chứng minh.

**Delivery Type:** Verification/promotion — không triển khai item, loot, heal hoặc throwable feature.
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** FR30, FR34–FR50; NFR14, NFR27–NFR31.
**Depends on:** Story 3.1–3.11.
**Blocks:** Story 4.1, Story 4.13.
**Module/File Ownership:** Epic 3 scenario pack, evidence index và promotion decision.
**Platform Applicability:** Linux headless authority và representative Keyboard/Mouse/Touch clients.

**Acceptance Criteria:**

**Given** signed feature manifests/evidence từ Story 3.1–3.11  
**When** inventory→pickup→heal→loot→airdrop→throwable scenarios chạy dưới impairment  
**Then** không item duplication/loss, false progress hoặc client commit và mọi FR30/34–50 truy tới owner fixture  
**And** gate không sửa feature; failure trả về Story owner.

**Error/Recovery Path:** Missing/failing evidence giữ Epic 4 blocked với build/seed/command reproduction.
**Test Boundary:** Cross-feature deterministic flow, impairment, manifest/hash parity và trace completeness.
**Definition of Done:** Signed evidence pack đạt; Story 4.1 được mở mà không có feature implementation ẩn.
...
**Depends on:** Story 1.23, Story 2.15, Story 3.12.
...
**Depends on:** Story 4.3, Story 1.14–1.18.
...
**Blocks:** Story 4.13, Story 5.6, Story 8.8.
...
**And** spectator không có inventory riêng hoặc marker bí mật.

**Given** spectator đã bị loại  
**When** dùng communication  
**Then** spectator được nghe/nói trong team voice channel theo cùng mute/block/permission contract  
**And** không được tạo world/context ping, Map marker hoặc tactical marker mới; server trả `spectator_ping_forbidden`.
...
**Blocks:** Story 4.11–4.13, Story 6.15.
...
**Blocks:** Story 4.12–4.13, Story 9.1, Story 9.4.
...
**Blocks:** Story 4.13, Story 9.4–9.6.
...
**Delivery Type:** Verification/promotion — chỉ chạy full-loop/evidence và quyết định promotion; feature failure quay về Story sở hữu.
**Sizing:** M — tối đa 5 ngày cho matrix đã định nghĩa.

**Source Requirements:** FR1–FR12, FR31–FR33, FR90–FR91; GDD-IR-FR70; NFR1–NFR3, NFR19–NFR20, NFR27.

**Depends on:** Story 4.1–4.12, Story 1.23, Story 2.15, Story 3.12.

**Blocks:** Story 5.1–5.2, Story 6.5–6.13.
...
**Given** full-match telemetry batch  
**When** phân đoạn thời gian sống 0–3, 3–10, 10–22 và 22+ phút  
**Then** report phải chứng minh progression lần lượt là tự vệ, hoàn thiện hai vai trò + route, tối ưu resource/position và kỷ luật information/smoke  
**And** mỗi segment có metric owner; regression ngoài envelope tạo balance finding thay vì bị average toàn trận che khuất.
...
### Story 5.1: Minimal terrain-slice contract và deterministic bake

As a world developer,
I want schema/bake nhỏ nhất cho terrain slice đầu tiên,
So that contract phát triển theo consumer thay vì thiết kế trước toàn world domain.

**Delivery Type:** Enabler — consumer trực tiếp là Story 5.2; không tạo trước road/building/loot/vehicle/weather schema.
**Sizing:** S — tối đa 3 ngày.
**Source Requirements:** FR83–FR84; NFR29, NFR31, NFR46–NFR48, NFR55.

**Depends on:** Story 1.2–1.5, Story 4.13.

**Blocks:** Story 5.2–5.3.

**Module/File Ownership:** Minimal `TerrainSliceDefinition`, `TerrainTileData`, `game/tools/terrain_bake`, generated terrain manifest và tests.

**Platform Applicability:** Editor/bake pipeline, năm client và Linux headless server.

**Acceptance Criteria:**

**Given** height/coast/water/material source tối thiểu cho Story 5.2  
**When** bake  
**Then** contract chỉ chứa tile index, height/collision/material mask, water/coast reference và dependency metadata  
**And** Terrain3D `1.0.2` chỉ chạy editor rồi bake versioned `TerrainTileData`; POI/road/building/loot/vehicle/weather fields chưa được tạo.
...
**Given** runtime load terrain slice  
**When** manifest được resolve  
**Then** geometry là authored/baked và không được sinh lại từ match seed  
**And** schema extension sau chỉ được thêm trong Story consumer với version/migration/validator riêng.
...
**Test Boundary:** Minimal schema round-trip, deterministic terrain bake/hash, generated-edit guard và server package-content tests.

**Definition of Done:** Terrain slice tối thiểu bake/version/hash được; không có field của capability tương lai và có rule mở rộng theo consumer.
...
**Depends on:** Story 5.1 và Story 1.17, Story 1.22.
...
**Given** 36 texture candidate và subset đã duyệt ở Story 1.22  
...
**Blocks:** Story 5.4–5.7, Story 5.13, Story 7.2, Story 7.8.
...
**Given** streaming là consumer đầu tiên của world-cell data  
**When** schema được mở rộng từ Story 5.1  
**Then** chỉ `WorldCellId`, bounds, dependency/lease metadata và readiness state cần cho streaming được thêm  
**And** extension có migration/validator/hash parity, không kéo road/building/weather field.
...
**Source Requirements:** FR84–FR85, FR87; GDD-IR-FR76; NFR25, NFR31, NFR46.

**Depends on:** Story 5.1–5.3.

**Blocks:** Story 5.5–5.6, Story 5.13, Story 7.3.
...
**Given** hai high-loot POI và route graph  
**When** pacing validator chạy  
**Then** khoảng cách POI mục tiêu 800–1.500 m, có ít nhất hai tuyến mặt đất độc lập  
**And** mỗi POI lớn có đường rút không cần xe với travel time không chậm hơn main route quá 70%; report lưu route witness.
...
**Source Requirements:** FR84–FR85; GDD-IR-FR57, GDD-IR-FR77; NFR31, NFR35, NFR46–NFR48, NFR55.

**Depends on:** Story 5.4.

**Blocks:** Story 5.6, Story 5.13, Story 7.4.
...
**Given** building/module được promote cho Standard v1.0  
**When** combat-language validator chạy  
**Then** roof lợi thế có ít nhất hai access route hoặc một documented blind spot, và ≥80% combat window dùng approved size/silhouette family  
**And** penetrable/non-penetrable material có visual + stable material ID nhất quán trên desktop/mobile/server collision.

**Given** gameplay shell hoặc placement data  
**When** scope validator scan  
**Then** không được chứa destructible component/state, building destruction, terrain digging hoặc fortification/building capability  
**And** mọi violation block manifest thay vì bị ignore.
...
**Source Requirements:** FR84–FR86; GDD-IR-FR55, GDD-IR-FR57; NFR25, NFR31, NFR35.

**Depends on:** Story 5.2, Story 5.4–5.5 và Story 4.8.

**Blocks:** Story 5.13, Story 7.7.
...
**Given** sampled combat spaces trong nhà, làng/rừng, ruộng/sườn đồi/đường và long-range vantage  
**When** sightline histogram chạy trên gameplay collision  
**Then** envelope mục tiêu lần lượt là 0–25 m, 40–150 m, 150–400 m và 400–800 m chỉ tại vị trí có trade-off/counter-route rõ  
**And** outlier lưu tọa độ, origin/target, occluder và owner; visual-only occluder không được tính pass.

**Given** cover/terrain/building manifest  
**When** Standard-scope validator chạy  
**Then** cover phải tĩnh và không có destructibility, digging hoặc player-built fortification state  
**And** client/server/platform variants trả cùng collision/LOS result.
...
**Source Requirements:** FR87–FR89; GDD-IR-FR59; NFR29, NFR31, NFR35, NFR44.

**Depends on:** Story 5.1–5.3.

**Blocks:** Story 5.13, Story 7.9.
...
**Given** weather/time profile và bốn default outfit  
**When** fairness gate chạy  
**Then** Standard không có full-night profile; clear/rain/fog giữ target readable theo GDD  
**And** controlled target-detection study cho bốn outfit phải có detection-rate delta ≤5% trên approved desktop/mobile profiles.
...
**Depends on:** Story 1.18–1.19, Story 5.8–5.9.
...
### Story 5.13: Vùng thử nghiệm 2×2 km promotion evidence

As a người chơi,
I want đi bộ, bơi và đi thuyền xuyên slice ổn định,
So that world/vehicle/water loop được chứng minh trước bản đồ 8×8 km.

**Delivery Type:** Verification/promotion — chỉ tổng hợp traversal/vehicle/water/map evidence; feature failure quay về Story 5.1–5.12.
**Sizing:** M — tối đa 5 ngày.

**Source Requirements:** FR51–FR55, FR84–FR89; NFR31, NFR35, NFR44, NFR46–NFR48.

**Depends on:** Story 5.1–5.12, Story 4.13.

**Blocks:** Story 6.18–6.20 và Story 7.1.
...
**Definition of Done:** 2×2 slice đạt streaming/water/vehicle/map evidence, dùng asset hợp lệ và mở khóa network scale; gate không chứa feature implementation.
...
**Depends on:** Story 1.2–1.5, Story 2.1.

**Blocks:** Story 6.2–6.16.
...
**Blocks:** Story 6.3–6.7, Story 6.16.
...
**Blocks:** Story 6.6–6.11, Story 6.18–6.20.
...
**Depends on:** Story 1.13, Story 6.2, Story 6.5.

**Blocks:** Story 6.7, Story 6.11, Story 6.18–6.20.
...
**Blocks:** Story 6.18–6.20 và release integrity checks.
...
**Blocks:** Story 6.11, Story 6.18–6.20.

**Module/File Ownership:** `backend/allocator`, Agones/Kubernetes allocation adapters/contracts và allocation tests.
...
### Story 6.9: Linux headless container và lifecycle hardening

As an operations engineer,
I want match server artifact được đóng gói/harden ngay khi allocator tồn tại,
So that scale test dùng đúng artifact sẽ vận hành thay vì một image release tạo muộn.

**Delivery Type:** Enabler — consumer trực tiếp là secure join và scale gates.
**Sizing:** M — tối đa 5 ngày.
**Source Requirements:** NFR13, NFR17, NFR19–NFR20, NFR26, NFR49, NFR55.
**Depends on:** Story 1.3–1.5, Story 6.8.
**Blocks:** Story 6.11, Story 6.18–6.20 và Story 10.8.
**Module/File Ownership:** Linux headless export, multi-stage OCI image, non-root runtime, health/readiness/drain/SIGTERM và image SBOM.
**Platform Applicability:** Linux x64 match server, local OCI và Agones/Kubernetes.

**Acceptance Criteria:**

**Given** locked server artifact từ CI family  
**When** OCI image build/scan/boot  
**Then** image pin digest, chạy non-root với read-only rootfs/capability tối thiểu, chứa SBOM và không chứa client presentation asset/secret/toolchain thừa  
**And** local, CI và production allocation dùng cùng artifact digest.

**Given** Agones lifecycle hoặc SIGTERM  
**When** readiness/health/drain/shutdown chạy  
**Then** server ngừng nhận join, finalizes evidence/result theo bounded timeout và thoát sạch  
**And** missing config/short-lived secret fail trước Ready.

**Error/Recovery Path:** Scan/boot/lifecycle failure chặn image promotion và allocator giữ digest trước.
**Test Boundary:** Image content/CVE/SBOM, non-root/security, boot/readiness/health/drain/SIGTERM và artifact parity.
**Definition of Done:** Hardened server image là required input của secure join/scale, không còn deferred tới release Epic.

### Story 6.10: Online observability và rollback foundation

As an operations engineer,
I want telemetry/runbook xuất hiện cùng match service đầu tiên,
So that admission, allocation và server failure có thể phát hiện và rollback trước scale.

**Delivery Type:** Enabler — consumer trực tiếp là Story 6.11 và các scale gates; không đổi gameplay.
**Sizing:** M — tối đa 5 ngày.
**Source Requirements:** NFR19–NFR22, NFR28–NFR30, NFR51, NFR55; ADR-19.
**Depends on:** Story 1.2–1.4, Story 6.8–6.9.
**Blocks:** Story 6.11, Story 6.18–6.20, Story 10.11.
**Module/File Ownership:** `GameLog`, telemetry schema, allocation/join/server metrics, dashboards/alerts, canary/rollback runbook và drills.
**Platform Applicability:** Control plane, Linux match server và test clients.

**Acceptance Criteria:**

**Given** allocation→boot→join→match lifecycle  
**When** structured telemetry phát  
**Then** `trace_id`, `match_id`, account hash, build/region/protocol/content IDs liên kết được; hot path dùng bounded counter/histogram  
**And** raw voice, token, signing secret và PII không vào log/metric.

**Given** canary hoặc failure drill  
**When** alert/runbook kích hoạt  
**Then** owner/detection/pause/drain/rollback về signed compatible artifact/config family rõ và audit được  
**And** remote ops chỉ disable queue/region/provider, không đổi balance, pool, bot hoặc Standard rule.

**Error/Recovery Path:** Telemetry outage không mở authority; rollout pause/rollback an toàn và incident có explicit degraded state.
**Test Boundary:** Schema/cardinality/privacy, alert/runbook, crash/result drill, N/N-1 backend support và no-mixed-match compatibility.
**Definition of Done:** Online slice có dashboards/alerts/rollback drill trước secure admission/scale.

### Story 6.11: Secure join, seat claim và compatibility admission
...
**Depends on:** Story 2.1, Story 6.5–6.10.

**Blocks:** Story 6.15–6.20.
...
### Story 6.12: Team voice provider spike và VoiceProvider adapter
...
**Depends on:** Story 6.1–6.2, Story 6.11.

**Blocks:** Story 6.16, Story 8.12–8.13.
...
**Then** Story 6.12 và Epic 6 phải giữ trạng thái blocked cho tới khi một provider khác đạt cùng contract hoặc FR81 được thay đổi bằng quyết định sản phẩm chính thức  
...
**Given** participant chuyển sang eliminated teammate spectator  
**When** team voice permission resolve  
**Then** participant vẫn được nghe/nói trong đúng team channel với cùng mute/block/PTT/permission contract cho tới team eliminated hoặc leave  
**And** voice không mở proximity/enemy channel hay trao quyền ping chiến thuật.
...
### Story 6.13: Eight contextual team pings
...
**Depends on:** Story 1.19, Story 6.2, Story 6.11.

**Blocks:** Story 8.5, Story 8.14.
...
**Given** eliminated spectator gửi world/context/Map ping  
**When** server validate sender state  
**Then** command bị từ chối bằng `spectator_ping_forbidden`, không tạo marker/TTL/cooldown side effect  
**And** spectator vẫn nhận ping hợp lệ của teammate sống theo information parity.
...
### Story 6.14: Friendly fire và neutral attribution
...
**Depends on:** Story 2.6, Story 4.4, Story 6.2, Story 6.11.

**Blocks:** Story 9.5, Story 6.18–6.20.
...
### Story 6.15: Reconnect policy ADR và secure seat contract
...
**Depends on:** Story 4.10, Story 6.2, Story 6.11.

**Blocks:** Story 6.16, Story 6.18–6.20.
...
**Then** policy bắt buộc là: party disconnect clear Ready ngay; queue/loading/pre-aircraft giữ ticket/seat tối đa 60 giây rồi release không tạo match defeat; sau aircraft giữ same seat tối đa 120 giây  
**And** trong live grace server phát neutral input, avatar vẫn hoàn toàn vulnerable/lootable, không invulnerability/autopilot; chết/team-wipe/match-end trong grace thắng timeout và quyết định destination authoritative.

**Given** live-match grace 120 giây hết sau aircraft  
**When** chưa reconnect  
**Then** server áp leave-after-aircraft defeat, direct-eliminate avatar và drop loot tại authoritative position, không tạo DBNO/revive window do timeout  
**And** team còn sống tiếp tục; result/placement theo Standard, destination là `Timed out`, `Team eliminated` hoặc `Match ended`.

**Given** client vẫn connected nhưng không có semantic input  
**When** AFK threshold 120 giây đạt  
**Then** server chỉ gắn AFK telemetry/moderation signal và tiếp tục neutral command; không bảo vệ, di chuyển, bot-replace hoặc auto-kick avatar  
**And** transport loss sau đó mới bắt đầu grace tương ứng.
...
**Error/Recovery Path:** Owner không trả canonical timer/state thì client hiển thị reconnect unknown/degraded và không hứa bảo vệ; server vẫn dùng fixed policy trên.

**Test Boundary:** Transition/policy table exhaustive, token lifecycle/threat cases, phase/team outcomes và ownership contracts.

**Definition of Done:** ADR/policy trên được duyệt nguyên văn, không câu hỏi reconnect/AFK mở, schema/ports/tests khóa contract implementation.

### Story 6.16: Mobile interruption và reconnect state machine
...
**Depends on:** Story 6.12, Story 6.15.

**Blocks:** Story 6.18–6.20, Story 10.4–10.5.
...
### Story 6.17: Evidence Event Spine và authority hardening
...
**Depends on:** Story 2.15, Story 3.12, Story 4.13, Story 5.13, Story 6.9–6.16.

**Blocks:** Story 6.18–6.20, Story 9.5, Story 10.8, Story 10.10.
...
### Story 6.18: 24-client protocol-mixed scale gate

As a người chơi,
I want trận 24 người giữ tick và fairness,
So that hệ thống chứng minh bước scale đầu tiên.

**Delivery Type:** Verification/benchmark — chỉ tạo load/correctness evidence và promotion decision.
**Sizing:** M — tối đa 5 ngày cho scenario/harness đã tồn tại.

**Source Requirements:** FR71–FR82, FR109–FR110; NFR13–NFR17, NFR21, NFR31, NFR52.

**Depends on:** Story 6.1–6.17, Story 5.13.
...
### Story 6.19: 50-client protocol-mixed scale gate

As a người chơi,
I want trận 50 người ổn định dưới tải trung gian,
So that relevance/replication được chứng minh trước target 100.

**Delivery Type:** Verification/benchmark — không chứa replication feature chưa có Story owner.
**Sizing:** M — tối đa 5 ngày.

**Source Requirements:** FR71–FR82, FR109–FR110; NFR13–NFR17, NFR21, NFR31, NFR52.

**Depends on:** Story 6.18.
...
### Story 6.20: 100-client Standard online gate
...
**Delivery Type:** Verification/benchmark — tạo evidence/capacity decision, không che feature implementation.
**Sizing:** M — tối đa 5 ngày.

**Source Requirements:** FR71–FR82, FR109–FR110; NFR13–NFR17, NFR19–NFR22, NFR31–NFR35, NFR51–NFR53.

**Depends on:** Story 6.19.

**Blocks:** Story 7.1, Story 10.8 và Story 10.11.
...
**Delivery Type:** Enabler/promotion — khóa scope/content budget bằng evidence, liên kết trực tiếp tới Story 7.2–7.10.
**Sizing:** M — tối đa 5 ngày.

**Source Requirements:** FR83; NFR46–NFR53.

**Depends on:** Story 5.13 và Story 6.20 đạt, không chỉ có waiver.
...
**Source Requirements:** FR83–FR85; GDD-IR-FR76; NFR25, NFR31, NFR46.
...
**Given** high-loot POI graph toàn map  
**When** spacing/escape validator chạy  
**Then** POI mục tiêu cách nhau 800–1.500 m, mỗi pair/POI trọng yếu có ít nhất hai ground route  
**And** mỗi POI lớn có non-vehicle escape không chậm hơn main route quá 70%, kèm route witness.
...
**Source Requirements:** FR83–FR85; GDD-IR-FR57, GDD-IR-FR77, GDD-IR-FR79; NFR31, NFR35, NFR46–NFR48, NFR55.
...
**Given** production library/art contract  
**When** scope/combat-language validation chạy  
**Then** không destructibility/digging/building state; roof advantage có ≥2 access hoặc blind spot, ≥80% combat window thuộc approved silhouette family và penetration material đọc nhất quán  
**And** art giữ restrained realism, ordinary-survivor/world silhouette và fictional identity; decorative variant không tạo camouflage vượt fairness gate.
...
**Source Requirements:** FR83–FR85; GDD-IR-FR77; NFR31, NFR35, NFR46–NFR48, NFR50.
...
**Given** ≥420 placement batch  
**When** combat-window/material/roof validator tổng hợp  
**Then** toàn map vẫn giữ ≥80% approved window family, roof counterplay và penetration visual/material-ID contract  
**And** placement outlier có owner/fix evidence, không được average che critical violation.
...
**Source Requirements:** FR84–FR86; GDD-IR-FR55, GDD-IR-FR76; NFR25, NFR31, NFR35.
...
**Given** sampled indoor/village-forest/open-field/long-vantage spaces  
**When** full-map sightline histogram chạy  
**Then** distribution tuân envelope 0–25/40–150/150–400/400–800 m, long range chỉ pass khi có trade-off/counter-route  
**And** POI escape/spacing rules từ Story 7.3 được kiểm lại trên collision/navigation đã bake.
...
**Source Requirements:** FR83–FR89; GDD-IR-FR78, GDD-IR-FR85; NFR44, NFR46–NFR48.
...
**Given** player điều hướng không mở minimap  
**When** traversal study chạy giữa POI/compound/route chính  
**Then** landmark, sign, material/road/coast language cho phép xác định route bằng world cues với approved success threshold  
**And** test không dùng hidden HUD/minimap hint để pass.

**Given** full-map audio propagation profile  
**When** range/masking measurement chạy  
**Then** sprint/run/walk footsteps đạt 70/45/20 m; unsuppressed gunshot đạt tới 1.000 m, suppressed 400–600 m theo approved weapon profile  
**And** surface material có distinct spectrum và aircraft/airdrop ambience không che hoàn toàn nearby gunfire.
...
**Source Requirements:** FR86–FR91; GDD-IR-FR59; NFR25, NFR31, NFR35, NFR44.
...
**Given** all approved time/weather profiles và bốn default outfit  
**When** full-map detection study chạy  
**Then** không profile full-night và outfit detection-rate delta ≤5% trong representative clear/rain/fog spaces  
**And** failed profile/outfit bị block hoặc sửa art, không nới fairness threshold.
...
**Delivery Type:** Verification/promotion — chỉ ký manifest từ evidence Story 7.1–7.9, không triển khai content còn thiếu.
**Sizing:** M — tối đa 5 ngày.
...
### Story 8.1: Field Instrument Theme hardening và component expansion
...
**Definition of Done:** Foundation từ Story 1.7 được harden thành Theme/component production reusable; không phải lần đầu gameplay UI nhận presenter.
...
**Depends on:** Story 1.18, Story 1.21, Story 8.1.
...
### Story 8.3: Việt/Anh localization và Noto production hardening
...
**Depends on:** Story 1.8, Story 8.1–8.2.
...
**Definition of Done:** StringId/catalog foundation từ Story 1.8 được harden production; Việt/Anh/Noto đầy đủ, không missing glyph/crop critical text.
...
**Depends on:** Story 1.7–1.8, Story 1.18–1.21, Story 2.12, Story 4.8, Story 8.1–8.3.
...
**Depends on:** Story 1.19, Story 8.5.
...
**Depends on:** Story 3.2–3.6, Story 8.1–8.3, Story 8.5.
...
**Depends on:** Story 4.4, Story 4.8, Story 6.13, Story 8.1–8.3.
...
**And** Full Map giữ north-up cố định trong Standard; minimap dùng player-heading-up.

**Given** pan/zoom trên desktop hoặc mobile  
**When** transform map thay đổi  
**Then** zoom bị clamp 0,75×–6× theo authored map extent và pan bị clamp để ít nhất 20% authored bounds còn trong viewport  
**And** mọi platform dùng cùng world-information extent; zoom không tải loot/enemy/sound layer bị cấm.
...
**Depends on:** Story 1.20–1.21, Story 8.1–8.3.
...
**Depends on:** Story 1.21, Story 8.2, Story 8.9.

**Blocks:** Story 8.19, Story 10.4–10.5.
...
**Source Requirements:** FR104–FR105; GDD-IR-FR85; NFR31, NFR44–NFR45, NFR55; UX-DR49.

**Depends on:** Story 2.10, Story 5.7, Story 5.9, Story 8.9.

**Blocks:** Story 8.13–8.14, Story 8.19, Story 10.7, Story 10.11.
...
**And** player-facing controls chỉ gồm `Master`, `World`, `Team Voice`, `UI`, `Music`; `Ambience/Footsteps/Weapons/Vehicles` là internal child bus dưới `World`, giữ fixed fair ratio và không expose footstep boost/EQ/night mode.

**Given** competitive range/masking fixture  
**When** audio mix/attenuation được đo  
**Then** sprint/run/walk footsteps đạt 70/45/20 m; unsuppressed gunshots tới 1.000 m, suppressed 400–600 m theo weapon profile  
**And** surface material có distinct spectrum và aircraft/airdrop ambience không che hoàn toàn nearby gunfire.
...
**Depends on:** Story 6.12, Story 8.9, Story 8.12.
...
**Depends on:** Story 6.13, Story 8.3, Story 8.5–8.6, Story 8.12.
...
**Depends on:** Story 1.23, Story 2.15, Story 3.12, Story 8.4–8.6.
...
**Depends on:** Story 8.4, Story 8.15, Story 4.13.
...
**Blocks:** Story 9.1–9.8, Story 10.1–10.7, Story 10.11.
...
**Depends on:** Story 4.11, Story 6.17, Story 8.19.
...
**Depends on:** Story 6.14, Story 6.17, Story 9.1, Story 9.4.
...
**Source Requirements:** FR39, FR93, FR96–FR97; GDD-IR-FR65, GDD-IR-FR93; NFR1, NFR3, NFR24, NFR54; UX-DR11, UX-DR31, UX-DR56.

**Depends on:** Story 9.2–9.6, Story 8.19.

**Blocks:** Story 9.8, Story 10.11.
...
**Given** Standard v1 scope manifest  
**When** product-scope gate chạy  
**Then** Standard là ruleset chính; ranked, public custom server, tournament/esports tools và FPP-only queue được đánh dấu deferred  
**And** không route/config/schema ẩn nào có thể bật chúng trong v1 mà không qua course-correction + population/fairness evidence.

**Given** bất kỳ post-release feature/content proposal  
**When** integrity review  
**Then** proposal phải khai báo trụ cột P1–P4 được củng cố, metric tension/fairness/readability và playtest evidence owner  
**And** bị reject hoặc tách khỏi Standard nếu evidence cho thấy giảm tension, fairness hoặc readability; revenue/engagement không đủ để approve.
...
**Delivery Type:** Verification/promotion — chỉ tổng hợp E2E/privacy/product evidence; không triển khai profile/moderation feature mới.
**Sizing:** M — tối đa 5 ngày.
...
Người chơi nhận được bản build tương thích, an toàn, đạt performance/reliability trên Windows, Linux, macOS, Android và iOS. Epic này chỉ chứa qualification, hardening và release outcome: dependency lock/SBOM/CI/signing boundary đã thuộc Story 1.3–1.5; Linux container và observability/rollback đã thuộc Story 6.9–6.10.

### Story 10.1: Windows x64 release qualification
...
**Depends on:** Story 1.3–1.5, Story 8.19, Story 9.8.

**Blocks:** Story 10.6, Story 10.11.
...
**Then** thu frame/RAM/load metrics cho Story 10.6 và installer/content size nằm desktop ≤25 GB  
...
### Story 10.2: Linux x64 client release qualification
...
**Then** metrics đi vào Story 10.6, package trong desktop size budget  
...
### Story 10.3: macOS Universal 2 signing và notarization
...
**Then** parity đạt và metrics đi Story 10.6  
...
### Story 10.4: Android ARM64 AAB và device qualification
...
**Depends on:** Story 1.3–1.5, Story 6.16, Story 8.11–8.19, Story 9.8.

**Blocks:** Story 10.7, Story 10.11.
...
### Story 10.5: iOS ARM64 archive và TestFlight qualification
...
### Story 10.6: Desktop performance, memory và load-time gate
...
**Depends on:** Story 7.10, Story 8.19, Story 9.8, Story 10.1–10.3.
...
### Story 10.7: Mobile performance, memory và thermal gate
...
**Depends on:** Story 7.10, Story 8.19, Story 9.8, Story 10.4–10.5.
...
### Story 10.8: 100-player server load và correctness soak
...
**Depends on:** Story 6.9–6.10, Story 6.20, Story 7.10.
...
### Story 10.9: Network, streaming và patch-size hardening
...
**Depends on:** Story 1.3–1.5, Story 6.10, Story 7.10, Story 10.6–10.8.
...
### Story 10.10: Security, privacy, retention và account deletion
...
**Depends on:** Story 1.3, Story 6.10–6.17, Story 9.5–9.6.
...
### Story 10.11: Five-platform release-candidate acceptance
...
**Delivery Type:** Verification/release decision — chỉ tổng hợp evidence từ Story owner; không triển khai control kỹ thuật mới.
**Sizing:** M — tối đa 5 ngày cho RC matrix đã định nghĩa.

**Source Requirements:** FR1–FR110; NFR1–NFR55; UX-DR1–UX-DR56.

**Depends on:** Story 1.23, Story 2.15, Story 3.12, Story 4.13, Story 5.13, Story 6.20, Story 7.10, Story 8.19, Story 9.8, Story 10.1–10.10.
